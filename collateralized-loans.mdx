...
We are officially the first system that solves the $2 trillion problem: making illiquid security tokens useful for borrowing. Transform baskets of ERC1400 security tokens into fungible, DeFi-compatible collateral through our Asset Wrapper System.
...
Our Asset Wrapper System transforms illiquid security tokens into highly liquid, fungible assets that work seamlessly with overcollateralized lending. Its like an ERC20-based index fund backed by RWAs and fully on-chain.

<Steps>
  <Step title="Deposit Security Tokens">
    Deposit your ERC1400 security tokens into a vault. Real estate, bonds, equity, and other assets create a diversified basket. Security tokens created with our tools can be added instantly with one click. The vault accepts tokens created with our security token tool or partner tools that use our oracle.
  </Step>
...
<Warning>
Please note: The asset vault can only accept ERC1400-based tokens from issuers that have implemented our RWA orcale (Trusset Verify) into their tokenization service, otherwise only our platform-internal created tokens can be used.
</Warning>
...
<Info>
All assets can be liquidated by selling them. Auctions only happen for high-quality assets that meet specific criteria (currently private). These assets are mainly bought by our company to passively build an asset portfolio at lower rates
</Info>
